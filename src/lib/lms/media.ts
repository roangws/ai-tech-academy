...
/*
  ...
  These moved to lib/lms/format.ts because this module imports the Supabase
  server client, which imports `next/headers` — so a client component reaching
  for a timecode formatter dragged `next/headers` into the browser bundle and
  failed the build. Client components must import them from format.ts directly;
  the re-export exists so server callers have one obvious place to look.
*/
export { fileSize, timecode, spokenDuration } from "@/lib/lms/format";
