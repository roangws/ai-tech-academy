...
import { listCertifications, verifyPath } from "@/lib/lms/certifications";
            ...
            {records.map((r) => (
              ...
              >
                ...
                <span className="t-meta text-right text-ink-secondary">
                  {/* The reference, as the link a stranger would follow. The
                      useful check an administrator makes is the outside world's
                      check, so the row offers that one rather than an internal
                      view of the same row. */}
                  <Link
                    href={verifyPath(r.reference)}
                    className="block tabular-nums text-accent no-underline underline-offset-4 hover:underline"
                  >
                    {r.reference}
                  </Link>
