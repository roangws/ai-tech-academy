...
export default async function AdminApplications() {
  ...
  /* Course titles, resolved once. The catalogue is a query now, so a lookup
     inside the render loop would be a promise per row. Drafts included: somebody
     can apply to teach a course that is still being written. */
  const titles = new Map((await getAdminCatalog()).map((c) => [c.id, c.title]));
