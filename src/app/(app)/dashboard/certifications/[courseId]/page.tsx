...
import { getMyCertification, verifyPath } from "@/lib/lms/certifications";
    ...
    <Container className="py-8 md:py-10">
      ...
      <div className="mt-6">
        <Certificate
          ...
          photoUrl={viewer.profile?.avatar_url ?? null}
          verifyUrl={`${brand.domain}${verifyPath(row.record.reference)}`}
        />
      </div>

      {/* The reference and the page it resolves on, as a link somebody can
          follow from the screen as well as read off the paper. It is the same
          URL printed on the document, so a reader who has the PDF and a reader
          who has this page arrive at the same place. */}
      <p className="t-body-sm mt-5 max-w-[62ch] text-ink-secondary print:hidden">
        Print to PDF to save or send it. Anyone can check it at{" "}
        <Link
          href={verifyPath(row.record.reference)}
          className="text-accent no-underline underline-offset-4 hover:underline"
        >
          {brand.domain}
          {verifyPath(row.record.reference)}
        </Link>
        , which shows the name, the course and the date, and nothing else about your
        account.
      </p>
    </Container>
  );
