 ...
 * ------------------------------------------------------------------ moved here
 *
 * This was /judge in its entirety until phase one made that page about events.
 * Nothing about it changed except the address and the guard: Roan's instruction
 * was "focus only on events for phase 1", which is a statement about what the
 * console opens on, not a request to take two working features out of the
 * product. A curriculum review is still the thing content.ts:2562 describes
 * first, and a submitted outcome sheet still has nowhere else to be scored.
 *
 * ------------------------------------------------------------ two jobs, both
 *
 ...
 *
 * The second half is /judge now. The first half is this page. The curriculum
 * review is one document per seat per term, written against the seat's own
 * `checks` sentence, which is the closest thing to a criterion the board has.
 * The scoring queue is what a learner "presents" that exists in this system:
 * their submitted outcome sheet.
 *
 * ------------------------------------------------------------- seat, not role
 *
 ...
 *
 * Events are the opposite and deliberately so — see the note on /judge.
 */
export default async function JudgeCurriculumPage() {
  /* `judge`, not `admin`. See the note on /judge: this line named the wrong role
     on all three judging screens, and the effect was that a seated judge could
     not open any of them. */
  const viewer = await requireRole("judge", "/judge/curriculum");
  ...
  if (!seat) {
    return (
      <Container className="py-12 md:py-16">
        <Breadcrumb />
        <h1 className="t-display mt-4 text-ink">Curriculum review</h1>
  ...
  return (
    <Container className="py-12 md:py-16">
      <Breadcrumb />
      <h1 className="t-display mt-4 text-ink">{seat.seat}</h1>
      ...
      <section aria-labelledby="curriculum-heading" className="mt-14">
        ...
        <p className="t-body-sm mt-2 max-w-[62ch] text-ink-secondary">
          Read the course this term, the lessons and the labs, against the sentence above. One
          review per term; filing again revises the one already there.
        </p>
