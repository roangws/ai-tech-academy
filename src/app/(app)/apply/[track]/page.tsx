...
import { byId, getMyApplication } from "@/lib/lms/queries";
import { reopenApplication, withdrawApplication } from "@/app/actions/apply";
import { advisors, apply, courses } from "@/lib/content";
      ...
      {editable ? (
        <>
          ...
          <ApplyForm
            ...
            courses={courses.map((c) => ({ id: c.id, badge: c.badge, title: c.title }))}
            profile={{
 ...
 * are worried about.
 */
function Summary({ application: a }: { application: Application }) {
  const items: readonly { label: string; value: string | null }[] = [
    ...
    /* The title, never the id. `course_id` is 'gtm' in the column and the summary
       ...
       gave. `byId` is the same catalog map the consoles use. */
    {
      label: "Course",
