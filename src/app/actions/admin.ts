...
export async function setModuleAccess(formData: FormData) {
  ...
  if (access === "account") {
    ...
    if ((open ?? []).length <= 1 && (open ?? []).some((m) => m.id === moduleId)) {
      throw new Error(
        "Every course keeps at least one open module. It is the open first-module promise the site makes on six surfaces, so open another module before closing this one.",
      );
    }
  }
